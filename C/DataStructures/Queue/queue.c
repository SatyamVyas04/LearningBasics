...
struct Queue* initialize_queue(unsigned size) {
  ...
  queue->rear = -1;
  queue->size = size;
  queue->array = (char*)malloc((size + 1) * sizeof(char));
  return queue;
}
...
void display(struct Queue* queue) {
  printf("\n> Queue Front\n");
  for (int i = 0; i < queue->size && queue->array[i] != '\0'; i++) {
    printf("> %c\n", queue->array[i]);
  }
  printf("> Queue Rear\n\n");
}
...
void main() {
  struct Queue* q = initialize_queue(20);
  printf("> %d\n", isEmpty(q));  // 1 for Empty Queue Array
  enqueue(q, 'a');               // Enqueue-d 6 characters
  enqueue(q, 'b');
  enqueue(q, 'c');
  enqueue(q, 'd');
  enqueue(q, 'e');
  enqueue(q, 'f');
  display(q);                       // Displaying
